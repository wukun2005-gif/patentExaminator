...
>
> 如与 `PRD.md` 冲突，以 PRD 为准。执行过程中如发现本文件自相矛盾或无法落地，必须先发起澄清，不得自行推测。

> **B-008 更新（2026-05-12）：** v0.1.0 产品方向已切换为复审 AI 助手，不再保留初审从零分析路径。执行顺序以 `B-008/b008_implementation_plan.md` 的 Phase A-E 为准：类型基础 → 复审 Agent → 复审面板/路由 → 既有模块复审适配 → 文档与 E2E。历史章节中仍保留的初审描述仅作为迁移前背景，不作为当前实现要求。
...
11.14. Backlog（AI 执行队列 — 22 个 Feature，按顺序 pick up）
...
4. **AI 交互式解读申请文件**：文字提取后，AI 以通俗语言解读技术方案、发明问题、关键技术手段、权利要求保护范围。审查员可自由追问（不限轮数），对话上下文持续累积。此为审查员快速理解专利的核心体验。
5. 上传单个或多个对比文件，并支持手动添加文献条目；支持多文件选择上传；Chrome/Edge 额外支持文件夹导入（File System Access API）；Firefox 用户通过多文件选择上传替代。
6. 自动或半自动提取对比文件公开日，并按基准日（优先权日 ?? 申请日）进行时间轴校验。
7. 对目标权利要求生成可编辑 Claim Chart，包含特征编号、特征描述、说明书出处、备注。
8. 对一篇"可用"对比文件生成新颖性对照，输出每条特征的公开状态 + Citation + 区别特征候选 + 待检索问题清单。

> **零对比文件分支：** 审查员未上传对比文件时（PRD §4 分支场景），Claim Chart 正常生成；新颖性/创造性面板显示"未上传对比文件，跳过对照"。Claim Chart Agent 的 `claimChartSchema` 增加 `pendingSearchQuestions` 字段（最多 5 条），零对比文件时由 claim-chart agent 基于权利要求特征生成待检索问题清单；后续补充对比文件并触发新颖性对照后，由 Novelty Agent 补充/覆盖该清单。
...
| Markdown 导出 | 壳子：有入口与占位 UI，不要求完整输出 |
| RAG 本地向量库 | v0.1.0 不实现；保留数据模型与接口占位 |
| 案件历史与交互记录追溯 | 壳子：案件列表 + 载入已有案件；v0.2.0 完整实现：按案件查看完整交互历史 / 按 moduleScope 分组 / 关键词搜索 / 导出交互历史（数据已存储在 IndexedDB） |
...
| Firefox ≥ 100 | 次等 | 无 File System Access API → 文件夹导入降级为"多文件选择"；用户版本可能非最新，需覆盖旧版测试 |
...
内网 / 本地机器（单台）
  app/ (打包产物)
    ...
    └── data/                    本地数据目录（server 进程可写）
         ...
         └── (OCR 缓存通过浏览器端 IndexedDB 存储，见 §6.4 ocrCache)
```
...
│       │   │   └── documentsSlice.ts
│       │   ├── interpret/
│       │   │   ├── InterpretPanel.tsx      # 文档解读展示 + 追问
│       │   │   └── interpretSlice.ts       # 解读状态管理（可选，也可复用 chatSlice）
│       │   ├── references/
...
│       │   │   └── settingsSlice.ts
│       │   ├── history/
│       │   │   ├── CaseHistoryPanel.tsx    # 案件历史列表（壳子）
│       │   │   ├── CaseListView.tsx        # 案件列表展示
│       │   │   └── historySlice.ts         # 历史状态管理（壳子）
│       │   └── mock/
│       │       ├── MockProvider.ts
...
│       └── prompts/
│           ├── interpret.prompt.md          # 文档解读 system prompt
...
| `client/src/lib/sanitizeSend.ts` | 外发前脱敏（sanitize）+ 返回后还原（restore） | `applySanitizeRules(text, rules)`, `restoreSanitizedText(text, map)` |
...
- 所有主键 `id` 使用 **UUIDv4**（`crypto.randomUUID()`，与 ADR-008 一致）。ULID 留作后续优化（若需要按 ID 排序的场景）。
- 时间统一使用 ISO8601 字符串：
...
阈值硬编码为 40（v0.1.0 不暴露到 `AppSettings`；后续可扩展为 `pdfMinCharsPerPage` 配置项）。
...
- 申请号：正则 `(CN)?\s*\d{9,13}[A-Z]?`（覆盖旧格式9位、新格式12–13位）/ 扉页"申请号"附近文字。
...
### 8.4.5 文档解读 Agent（interpret）

#### 触发时机

文字提取 / OCR 确认后，权利要求拆解之前。用户可跳过直接进入拆解。

#### 输入

```ts
interface InterpretInput {
  applicationText: string;        // 申请文件全文（经 TextIndex 裁剪）
  title: string;                  // 发明名称
  claimTexts: string[];           // 权利要求原文列表
}
```

- TextIndex 裁剪策略：优先保留权利要求全文 + 说明书前 30%（通常含技术领域、背景技术、发明内容）+ 摘要。超出 token 预算时截断说明书实施例部分。
- AgentClient 构造请求时 `moduleScope = "case"`。

#### Prompt 模板（`shared/src/prompts/interpret.prompt.md`）

System prompt 要点：
- 角色：专利技术解读助手，帮助审查员快速理解专利技术方案。
- 输出要求：使用通俗语言（非照搬原文），按固定结构输出：技术方案概述 → 发明要解决的问题 → 关键技术手段 → 独立权利要求保护范围解读 → 初步创新点观察。
- 法律边界：不得输出任何法律结论；创新点标注"仅供参考，不构成审查意见"。
- 自由文本输出（非 JSON），因为解读需要自然语言表达。

#### 对话交互

- 首次解读输出后，用户可在同一对话框中追问，上下文持续累积。
- 追问走 `chat` Agent（`moduleScope: "case"`），system prompt 包含当前案件上下文摘要和对话规则。
- 消息历史截断至最近 10 轮（与 DESIGN §6.4 一致）。

#### temperature / maxTokens

| 参数 | 值 | 理由 |
|------|-----|------|
| temperature | 0.3 | 通俗理解需自然语言表达，允许轻微发散 |
| maxTokens | 2000 | 首次解读输出较长 |
...
- **答复审查**（复用 `features/documents/DocumentUploadPanel.tsx`，通过 `documents.role = "office-action-response"` 区分）：仅提供"上传 + 展示"占位，不做分析。
...
| scope | 注入上下文 |
|---|---|
| `case` | 案件基线字段 + 申请文件全文（按 token 预算裁剪） |
...
- 测试钩子：`?mockDelay=fast` 固定 200ms（快速演示，与 PRD §6.11 一致）；`?mockDelay=0` 或 `window.__PATENT_MOCK_DELAY__ = 0` 跳过延迟（E2E 测试用）。
...
- 按 Unicode 字符数计（与 PRD §6.10 一致），中文字符计 1 个字符；`maxLen` 默认 40。
- 非法字符替换为 `_`；空白折叠为 `_`。
...
| T-EXPORT-004 | G1 ViewModel → `renderCaseMarkdown`（壳子） | 返回非空字符串，包含占位提示文案；不要求完整表格格式 |
...
| M10 | 评测集全量通过 + 验收 + 文档定稿 | M8, M8.5, M9 | 1.0 人日 |

> 合计 ≈ **21.5 人日**。若并行执行，优先级：M0 → M1 → （M2 ‖ M3）→ M4 → M5 → （M6 ‖ M8）→ M7 → M8.5 → M9 → M10。
...
### 11.10.5 M8.5 文档解读模块 + 案件历史壳子 + 浏览器检测

- **产出物**：
  - `shared/src/prompts/interpret.prompt.md`：文档解读 system prompt（通俗语言、结构化输出、法律边界约束）。
  - `features/interpret/InterpretPanel.tsx`：解读输出展示 + 追问输入框。复用 `AgentChatPanel.tsx` 通用对话组件，`moduleScope = "case"`。
  - `features/history/CaseHistoryPanel.tsx` + `CaseListView.tsx` + `historySlice.ts`（壳子）：案件列表展示，从 `cases` store 读取已有案件，点击可载入继续工作。
  - `components/BrowserNotice.tsx`：首次访问时检测浏览器类型，Firefox 用户显示非阻断性提示条。
  - 路由 `/cases/:caseId/interpret` + `/cases` 注册。
  - AgentClient 新增 `interpret` Agent 路由（Mock 模式走 fixture，真实模式走 Gateway）。
  - Mock fixture：G1/G2/G3 各增加 `agentResponses["interpret"]` 预置解读响应。
- **进入门槛**：M3（文档导入 Pipeline）+ M8（AI Gateway）完成。
- **离开门槛**：
  - Mock 模式：上传申请文件 → 点击"解读此专利" → 约 1-2 秒后返回预置解读 → 追问一轮 → 正常回复。E2E 冒烟通过。
  - 真实模式（可选验证）：配置 Token Plan Key 后，上传文件 → 解读 → 追问一轮 → 真实 AI 回复。
  - 案件历史列表：已有案件可展示并点击载入。
  - Firefox 用户访问时显示浏览器提示条。
...
### 11.14 Backlog（AI 执行队列）

> **用途：** 供后续 AI（或工程师）按顺序逐个 pick up feature，每完成一个 feature 提交一次，直到 backlog 全部开发完成。
>
> **执行规则：**
> 1. **严格按编号顺序执行**（B01 → B02 → … → B22），除非依赖关系允许跳过（见"前置依赖"列）。
> 2. **每个 Backlog Item 定义了一个可独立提交的 feature**——完成 Definition of Done 后立即 `git commit`，再 pick 下一个。
> 3. **提交前必须满足 §13.2 的 14 条自检清单**（typecheck / lint / 测试全绿 / 法律结论零泄漏 / API Key 零明文等）。
> 4. **每个 commit message 格式：** `[Bxx] <Feature 名称>`，如 `[B01] 工程骨架搭建`。
> 5. 如果发现上游文档（PRD / DESIGN / 本文件）自相矛盾或无法落地，**必须先发起澄清，不得自行推测**。
> 6. 如果某一 Item 阻塞了多个后续 Item，优先完成阻塞项，必要时可与用户确认调整顺序。

### Backlog 总览

| 编号 | Feature | 对应 Milestone | 前置依赖 | 预估 | PRD/Design 引用 |
|------|---------|---------------|---------|------|----------------|
| B01 | 工程骨架 & 基线配置 | M0 | — | 1.0d | §2.2, §2.3, §3, §5.1 |
| B02 | 领域类型定义 & zod Schema | M1 | B01 | 0.5d | §6.1, §6.2, §6.3, §8.9.1, DESIGN §3 |
| B03 | IndexedDB 封装 & Repository 层 | M1 | B01 | 0.5d | §6.4, DESIGN §11 |
| B04 | Zustand Store 骨架 & 路由 | M1 | B02, B03 | 0.5d | §7.1, DESIGN §4.4 |
| B05 | 日期解析 & 时间轴规则 | M2 | B02 | 0.5d | §8.2, DESIGN §3.2 ADR-009 |
| B06 | 案件基线模块（表单 + 校验 + 持久化） | M2 | B04, B05 | 1.0d | PRD §6.1, DESIGN §4.3.1 |
| B07 | 文档导入 Pipeline（PDF/DOCX/TXT/HTML 解析 + TextIndex） | M3 | B02 | 1.5d | §8.1.1, §8.1.6, §8.1.7, PRD §6.3 |
| B08 | OCR 模块（Tesseract.js + 进度 + 质量评分 + 缓存） | M3 | B07 | 1.5d | §8.1.2–8.1.5, PRD §6.3, ADR-006, ADR-011 |
| B09 | 文献清单模块（导入 + 自动提取 + 时间轴校验 UI） | M4 | B06, B08 | 1.5d | §8.3, PRD §6.2, DESIGN §4.3.3 |
...
| B17 | 设置面板（Provider 配置 + Agent 分配） | M8 | B15 | 1.0d | §8.10, PRD §7.5, DESIGN §4.3 |
| B18 | 文档解读模块（AI 交互式理解 + 追问） | M8.5 | B08, B15 | 1.0d | §8.4.5, PRD §6.3.5, DESIGN §4.3.4.5 |
| B19 | 案件历史列表壳子 + 浏览器检测 | M8.5 | B04 | 0.5d | PRD §6.12, §7.4, DESIGN §4.3.7.5 |
| B20 | 导出模块（HTML + 文件名规则 + Markdown 壳子） | M9 | B12, B14 | 1.0d | §8.13, PRD §6.10, DESIGN §4.3.8 |
| B21 | 反馈系统 + Evaluation Fixture 补齐（A1/A2/A3/E1/E2/E3） | M9 | B12 | 0.5d | PRD §8, §7.4, DESIGN §8.4 |
| B22 | 评测集全量验证 + 验收 + 文档定稿 | M10 | B18, B20, B21 | 1.0d | §9.5, §10.10, PRD §8, §16 |
...
**目标：** 从零搭建 monorepo 工程，所有工具链就绪。

**产出物：**
- 仓库目录结构完整落位（§5.1 目录树，空文件可用 `// TODO` 占位）。
- 顶层 `package.json`（§2.2）+ `tsconfig.base.json`（§2.3）+ eslint / prettier 配置。
- 4 个 Vitest config（unit / integration / e2e / evaluation）+ `playwright.config.ts`。
- `client` / `server` / `shared` 各自 `package.json` + `tsconfig.json` + `tsconfig.build.json`。
- `client/vite.config.ts`（含 `/api` proxy）+ `server/src/index.ts`（Express 最小骨架 + 静态托管）。
- 根级 `README.md` 最小骨架。

**Definition of Done：**
- `npm install` 成功；`npm run typecheck` / `npm run lint` / `npm test` 全部 exit 0。
- `npm run dev` 并发启动 Vite (5173) + Express (3000)；浏览器 `/` 可见占位页面 + 顶部 `banner-mode`。
- `npm run build && npm start` 可访问 `http://localhost:3000/`。

---

#### B02 — 领域类型定义 & zod Schema

**目标：** 共享层类型契约就绪，前后端可引用。

**产出物：**
- `shared/src/types/domain.ts`：PatentCase / SourceDocument / ReferenceDocument / TextIndex / ClaimNode / ClaimFeature / NoveltyComparison / InventiveStepAnalysis / ChatMessage 等全部类型。
- `shared/src/types/agents.ts`：ProviderConnection / AgentAssignment / AppSettings。
- `shared/src/types/api.ts`：AiRunRequest / AiRunResponse。
- `shared/src/types/feedback.ts`。
- `shared/src/schemas/*.schema.ts`（6 个 zod schema）。

**Definition of Done：**
- `typecheck` 全绿。
- 单元测试：每个 schema 至少 1 条合法 parse + 1 条非法 reject 用例，全绿。

---

#### B03 — IndexedDB 封装 & Repository 层

**目标：** 浏览器端持久化就绪，各 store 的 CRUD 可用。

**产出物：**
- `client/src/lib/indexedDb.ts`（`idb` 封装：open / migrate / 版本管理）。
- `client/src/lib/repositories/*.ts`（9 个 repo：case / document / reference / claim / novelty / inventive / feedback / ocrCache / settings）。
- Schema 版本表与 DESIGN §11 逐字对齐。

**Definition of Done：**
- 集成测试 `tests/integration/repositories.test.ts`：每个 store 完整 `create → readAll → update → delete`，全绿。
- ocrCache 写入附 `createdAt`，读取时检查 7 天过期。
...
**产出物：**
- `client/src/store/index.ts`（聚合所有 slice）。
- 各 `features/*/Slice.ts` 骨架（case / documents / references / claims / novelty / inventive / chat / settings）。
- `client/src/router.tsx`（全部路由注册，见 DESIGN §4.4）。
- `client/src/App.tsx` + `client/src/components/AppShell.tsx`（三栏布局骨架）。
- `client/src/components/ModeBanner.tsx`（Mock/真实模式横幅）。

**Definition of Done：**
- 浏览器导航到各路由路径可见占位组件。
- ModeBanner 正确显示 "演示模式"。
...
**产出物：**
- `client/src/lib/dateParse.ts`：多格式解析 + confidence。
- `client/src/lib/dateRules.ts`：`computeBaselineDate` + `classifyReferenceDate`。
- 单元测试 T-DATE-001..007 / T-PARSE-001..005。

**Definition of Done：**
- 全部日期相关单元测试全绿（包括优先权日优先于申请日、同日不可用、不确定公开日等边界）。

---

#### B06 — 案件基线模块

**目标：** 审查员可新建/编辑案件，填写核心字段，数据持久化。

**产出物：**
- `features/case/CaseBaselineForm.tsx` + `caseSlice.ts` + `caseValidation.ts`。
- 保存防抖 400ms 写入 IndexedDB。
- `applicationDate` / `priorityDate` 变更联动 `recomputeAllReferenceTimeline()`（此时可为 noop + TODO）。

**Definition of Done：**
- 表单字段全部可编辑，校验规则生效（申请日必填、优先权日 ≤ 申请日等）。
- E2E：填入 `input-application-date = 2023-03-15` / `input-title = 测试发明名称`，刷新后值仍保留。

---

#### B07 — 文档导入 Pipeline

**目标：** 支持 PDF/DOCX/TXT/HTML 文件解析 + TextIndex 建立 + Citation 四级匹配。

**产出物：**
- `lib/pdfText.ts`（pdfjs 抽取 + hasTextLayer 检测）。
- `lib/docxText.ts`（mammoth 封装）。
- `lib/htmlText.ts`（DOMParser）。
- `lib/textIndex.ts`（段落/页/行索引）。
- `lib/citationMatch.ts`（四级容错匹配）。
- `lib/fileHash.ts`（SHA-256）。
- `features/documents/DocumentUploadPanel.tsx` / `documentsSlice.ts`。
- 集成测试 T-DOC-001..003。

**Definition of Done：**
- 上传含文字层 PDF → 正确提取文本 + TextIndex。
- 上传 DOCX → 正确提取文本。
- 集成测试全绿。
...
**目标：** 扫描 PDF 自动 OCR、质量评分、缓存、用户确认流程。

**产出物：**
- `lib/ocrWorker.ts`（Tesseract.js Web Worker 封装 + 分页进度回调）。
- `features/documents/OcrProgressPanel.tsx` / `OcrReviewPanel.tsx`。
- OCR 质量评分（ADR-011 公式）。
- OCR 缓存（IndexedDB ocrCache，key = sha256(file)+lang+pageCount，7 天过期）。
- 语言包本地托管：`client/public/tessdata/chi_sim.traineddata` + `eng.traineddata`。
- 单元/集成测试 T-DOC-004..007。

**Definition of Done：**
- 无文字层 PDF → 自动启动 OCR → 显示进度条 → 质量评分三档展示 → 用户确认。
- OCR 缓存命中测试通过：同文件二次上传触发 0 次 Tesseract 调用。
- OCR 失败路径（模拟 WASM 崩溃）→ 显示"无法识别"提示 → 允许手动粘贴。

---

#### B09 — 文献清单模块

**目标：** 对比文件导入、自动提取、时间轴校验 UI 完整可用。
...
**目标：** 权利要求自动解析 + Claim Chart 表格可编辑，Mock 模式可用。

**产出物：**
- `lib/claimParser.ts`（识别权利要求区域、独权/从权、引用链）。
- `features/claims/ClaimChartTable.tsx` / `ClaimChartActions.tsx` / `claimsSlice.ts`。
- `agent/AgentClient.ts` + `agent/contracts.ts` + `agent/tokenEstimate.ts`（真实模式暂返回 `throw Error("real-mode-not-ready")`）。
- `shared/src/prompts/claimChart.prompt.md`。
- 单元测试 T-CLAIM-001..005 / T-CHART-001..005。

**Definition of Done：**
- 解析权利要求文本 → 正确输出 ClaimNode[]（识别独权/从权/引用链）。
- Mock 模式下生成 Claim Chart → 表格可编辑 → citationStatus 正确。
- E2E-G1-003 通过（载入 G1 → 填基线 → 生成 Claim Chart）。

---

#### B11 — Mock Provider & Fixture

**目标：** Mock 模式完整可用，默认开启，覆盖所有 Agent。

**产出物：**
- `features/mock/MockProvider.ts` / `mockRouter.ts`。
- `shared/src/fixtures/g1-led.json` / `g2-battery.json` / `g3-sensor.json`。
- 所有 fixture 的 `agentResponses` 通过对应 schema 的 `safeParse`。
- Mock 延迟仿真（默认 800–2000ms，`?mockDelay=fast` 固定 200ms，`?mockDelay=0` 零延迟）。

**Definition of Done：**
- 默认启动进入 Mock 模式，顶部横幅常驻。
- G1/G2/G3 可从 UI 载入，各 Agent 响应正确返回。
- Mock 模式零网络请求（`fetch` spy 断言 `calls.length === 0`）。

---

#### B12 — 新颖性对照模块（Mock）

**目标：** 逐特征新颖性对照 + Citation + 区别特征候选 + 待检索问题清单。

**产出物：**
- `features/novelty/NoveltyComparisonTable.tsx` / `NoveltyAgentTrigger.tsx` / `noveltySlice.ts`。
- `shared/src/prompts/novelty.prompt.md`。
- fixture G1 的 `novelty:ref-d1:1` 填充完整。
- 集成测试 T-NOV-001..005。

**Definition of Done：**
- 触发条件正确（claim-chart-reviewed + 可用文献 + citationStatus !== not-found）。
- 不可用文献按钮 disabled，hover 显示原因。
- E2E-G1-004 / E2E-G1-005 / E2E-G1-006 全绿。
- 顶部法律免责声明横幅正确显示。
...
- `shared/src/prompts/inventive.prompt.md`。
- fixture G2 的 `inventive:1` 完整填充。
...
**目标：** 三个壳子模块入口与占位 UI 就绪。

**产出物：**
- `features/summary/SummaryPanel.tsx` + `shared/src/prompts/summary.prompt.md`。
- `features/draft/DraftMaterialPanel.tsx`（四分区：正文草稿 / AI 备注 / 分析策略 / 待确认事项）。
- `features/defects/DefectPanel.tsx`（G3 显示"参数范围支持不足风险"占位）。

**Definition of Done：**
- 各模块导航入口可见，点击后展示占位内容。
- G3 fixture 的 `defectHints` 正确展示。
- 四分区严格分离。
...
**Definition of Done：**
- POST `/api/ai/run` 正常转发到 Provider Adapter。
- 429 配额错误 → 立即 fallback 到下一个 Provider。
- 5xx → 指数退避重试最多 2 次 → 失败后 fallback。
- 401 → 不重试不切换。
- API Key 默认仅存 server 内存；可选持久化 AES-256-GCM。
- 集成测试全绿。
...
**目标：** 真实模式每次外发不可绕过，Mock/真实切换安全。

**产出物：**
- `components/ExternalSendConfirm.tsx` / `ConfirmModal.tsx`。
- 模式切换安全提示 + 二次确认流程。
- AgentClient 真实模式分支接入 Gateway（移除 B10 的 throw placeholder）。
- E2E 测试 T-SEC-001..009。

**Definition of Done：**
- 真实模式下调用 `runAgent` 必须经过 ExternalSendConfirm；绕过则抛错。
- 模式切换弹窗正确显示安全提示。
- `localStorage` 中无 API Key 明文（T-SEC-009）。
- E2E 全绿。
...
**目标：** 用户可配置 Provider / Agent 分配 / 脱敏规则。

**产出物：**
- `features/settings/ProvidersConfigPanel.tsx`（Provider 连接列表 + API Key 输入）。
- `features/settings/AgentsAssignmentPanel.tsx`（Agent → Provider/Model 分配）。
- `features/settings/settingsSlice.ts`。
- 路由 `/settings`。

**Definition of Done：**
- UI 可添加/编辑/删除 Provider 连接。
- Agent 分配可选 Provider 顺序与 Model。
- 配置持久化到 IndexedDB `settings` store。
...
**目标：** AI 以通俗语言解读专利 + 审查员可追问。

**产出物：**
- `shared/src/prompts/interpret.prompt.md`。
- `features/interpret/InterpretPanel.tsx`（解读输出 + 追问输入框）。
- 复用 `AgentChatPanel.tsx`，`moduleScope = "case"`。
- Mock fixture：G1/G2/G3 各增加 `agentResponses["interpret"]`。
- AgentClient 新增 `interpret` Agent 路由。
- 路由 `/cases/:caseId/interpret`。

**Definition of Done：**
- Mock：上传文件 → "解读此专利" → 1–2 秒返回预置解读 → 追问一轮 → 正常回复。
- 真实模式（可选验证）：配置 Token Plan Key 后解读 + 追问正常。
- 对话记录持久化到 IndexedDB。
...
**目标：** 可浏览已有案件并载入 + Firefox 浏览器检测提示。

**产出物：**
- `features/history/CaseHistoryPanel.tsx` / `CaseListView.tsx` / `historySlice.ts`（壳子）。
- `components/BrowserNotice.tsx`。
- 路由 `/cases`。

**Definition of Done：**
- 已有案件可从 `cases` store 读取并展示列表，点击可载入。
- Firefox 用户访问时显示非阻断性提示条。
- E2E 冒烟通过。
...
**目标：** HTML 导出可打印 + 文件名自动生成。

**产出物：**
- `lib/exportHtml.ts` / `exportMarkdown.ts`（壳子）/ `fileNameSanitize.ts`。
- `features/export/ExportPanel.tsx` / `exportSlice.ts`。
- 导出内容组装：案件基线 → 文件清单 → Claim Chart → 新颖性对照 → 区别特征候选 → 待检索问题。
- 顶部固定法律免责声明。

**Definition of Done：**
- 导出 HTML 可直接打印，样式朴素清晰。
- 文件名规则：`{申请号}_{发明名称(≤40字)}_{类型}_{日期}.html`。
- 非法字符替换、长度限制、同日冲突追加序号全部正确。
- T-EXPORT-001..004 / E2E-G1-006 全绿。

---

#### B21 — 反馈系统 + Evaluation Fixture 补齐

**目标：** like/dislike/comment 可用 + 9 条评测 fixture 全部就位。

**产出物：**
- `components/FeedbackButtons.tsx` + `feedbackRepo.ts` 联动。
- Feedback 挂载到 ClaimChart / NoveltyRow / ChatMessage。
- 补齐 A1/A2/A3/E1/E2/E3 fixture。
- E2E 测试：`a1-functional.spec.ts` / `a2-boundary-date.spec.ts` / `a3-pct-priority.spec.ts` / `e1-no-reference.spec.ts` / `e2-scanned-pdf.spec.ts` / `e3-multi-independent.spec.ts`。

**Definition of Done：**
- 反馈 like/dislike/comment 可持久化，刷新页面后保留。
- 9 个 fixture 的 `agentResponses` 全部通过 schema `safeParse`。
- A1..A3 / E1..E3 E2E 全绿。
...
**目标：** v0.1.0 正式交付，所有验收项通过。

**产出物：**
- `tests/evaluation/runner.ts` / `metrics.ts` / `cases/*.test.ts`。
- `README.md` 完整版 + `CHANGELOG.md` 初始条目。
- `npm run verify` 全绿 + 耗时基线记录。

**Definition of Done：**
- `npm run verify` 全绿（typecheck + lint + unit + integration + e2e + evaluation）。
- Evaluation G1/G2/G3/A2/A3/E1/E3 自动评分达标（§9.5 阈值）。
- A1/E2 自动部分达标 + 人工复核签字。
- §16 验收清单逐项勾选完成。
- `DESIGN.md` 可追溯矩阵全部 ✅（MVP 依赖项）。

---

### Backlog 进度追踪

> 执行者在完成每个 Backlog Item 后，在下表打勾。此表为全局进度看板。

| 编号 | Feature | 状态 | 提交 SHA | 日期 |
|------|---------|------|---------|------|
...
| B16 | 外发确认弹窗 & 模式切换安全流 | ✅ | 30a4191 | 2026-05-10 |
| B17 | 设置面板 | ✅ | f0290b1 | 2026-05-10 |
| B18 | 文档解读模块 | ✅ | b622f87 | 2026-05-10 |
| B19 | 案件历史列表壳子 + 浏览器检测 | ✅ | 90f4672 | 2026-05-10 |
| B20 | 导出模块 | ✅ | b5cb144 | 2026-05-10 |
| B21 | 反馈系统 + Evaluation Fixture 补齐 | ✅ | 01275e8 | 2026-05-10 |
| B22 | 评测集全量验证 + 验收 + 文档定稿 | ✅ | 75ed029 | 2026-05-10 |
...
| §6.12 | 案件历史与交互记录（壳子） | §11.10.5 | `features/history/CaseHistoryPanel.tsx`、`CaseListView.tsx`、`historySlice.ts` | E2E 冒烟（案件列表展示 + 载入） |
...
| §7.4 UI | 中文 / 朴素 / 数据持久化预填 / 独立对话 / Token 预估 / 浏览器检测 | §7.1 / §8.8 / §8.9.4 / §3.4 | `features/chat/*`、`agent/tokenEstimate.ts`、`components/ModeBanner.tsx`、`components/BrowserNotice.tsx`、IndexedDB repo | §10.7 相关 E2E |
| §7.5 两级配置 | Provider + Agent 分配 + Fallback | §6.3 / §8.9 | `features/settings/*`、`server/src/providers/*` | T-GW-001..006 / T-DAI-001..006 |
| §7.6 资源库优化 | SHA-256 去重 / maxTokens 路由 | §8.1.5 / §8.9.5 | `lib/fileHash.ts`、`agent/contracts.ts` | T-DOC-007 / Mock 中 maxTokens 校验（单元） |
...
| 3. AI 交互式解读申请文件（通俗理解 + 追问） | E2E 冒烟（Mock 解读 + 追问） |
| 4. 填写申请号 / 申请日 / 优先权日 | E2E-G1-002 |
...
- Body（可选）解释"为什么"，引用 `DEVELOPMENT_PLAN.md §X.Y` 或 `PRD.md §X.Y`。
...
- [ ] A3 AI 交互式解读申请文件：点击"解读此专利"后返回通俗语言解读，可追问不限轮数。
- [ ] A4 填写申请号 / 发明名称 / 申请日 / 优先权日（可选） / 目标权利要求编号。
...
- [ ] D6 案件历史列表页面存在，可展示已有案件并点击载入。
- [ ] D7 Firefox 用户访问时显示浏览器检测提示条（非阻断性）。
...
- [ ] I4 `PRD.md` 与 `DEVELOPMENT_PLAN.md` 无误被 lockfile 以外手段修改（若需修改必须 PR 说明并记入 CHANGELOG）。
...
签字栏写入 `DESIGN.md` 的 "v0.1.0 验收记录" 小节；本 `DEVELOPMENT_PLAN.md` 不保存签字结果。

---

<p align="center"><em>— 本文件到此结束。如与 <code>PRD.md</code> 有任何冲突，以 PRD 为准；执行过程中发现本文件自相矛盾，必须先发起澄清，不得自行推测。 —</em></p>
