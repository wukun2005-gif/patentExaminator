...
| 核心诉求 | 减少重复性阅读和整理；不引入不熟悉的大工具；审查结论主导权在自己手里 |
...
她直接把扫描版 PDF 拖入上传区。工具检测到无文字层，自动启动 OCR，进度条显示"正在识别文字内容…"（约 15–30 秒）。OCR 完成后提示"已识别 XX 页，请确认文字质量"，她快速扫了一眼，点击确认。
...
**整个过程耗时：约 12 分钟**（其中 OCR 约 1 分钟，AI 分析约 3 分钟，人工确认和编辑约 8 分钟）。
...
flowchart TD
    ...
    DocAgent & ReadAgent & SumAgent & DraftAgent & HtmlAgent --> Orch
    Orch --> User
```
...
**输入方式：**
- 上传单个文件（.pdf / .docx / .txt / .html）
- 选择本地文件夹（浏览器 File System Access API，用户授权后遍历）
...
**OCR 规范：**
- 推荐使用 Tesseract.js（本地浏览器端运行，数据不外发）或服务端 Tesseract
- 支持中英文混合识别（专利申请常见）
...
**输出：每个技术特征的公开状态标注 + Citation（文件/段/行）**
...
**三步法结构（v0.1.0 以此为骨架，哪怕部分为壳子）：**

- **Step 1：** 确定最接近的现有技术（Closest Prior Art）
- **Step 2：** 确定区别技术特征 + 实际解决的技术问题
- **Step 3：** 判断技术启示（可组合多篇文件）

**约束：** APP 只输出候选判断和事实依据，最终创造性结论由审查员作出。
...
| 清楚简要 | 专利法§26.3；审查指南二·二章 | 壳子·手动标记 |
| 单一性 | 专利法§31；审查指南二·六章 | 壳子·手动标记 |
| 充分公开 | 专利法§26.3；审查指南二·二章 | 壳子·手动标记 |

### 6.7 专利申请简述（v0.1.0 可做·优先级低）
...
| 预置案例 | 对应测试集 | 涵盖流程 |
|---------|-----------|---------|
| G1：LED 散热装置 | 附录C G1 | 新颖性对照、时间轴校验、Citation |
| G2：锂电池快充方法 | 附录C G2 | 创造性三步法 Step 1/2/3 |
| G3：智能温控传感器 | 附录C G3 | 多从权、形式缺陷占位、零对比文件 |
...
| API Key 加密存储 | AES-256-GCM 加密；不明文存入 localStorage |
| 脱敏字段用户自定义 | sanitize → AI API → restore 可作为可选拦截层 |
| 默认本地/内网运行 | 不默认连接外部云服务 |
| OCR 本地运行 | OCR 优先在本地（浏览器端 Tesseract.js）运行，扫描内容不外发 |

### 7.2 性能

- 应用不能导致用户机器卡死；需与办公软件和浏览器共存
- 长任务（OCR、AI 分析）提供朴素进度提示
- 流式操作期间禁用主操作按钮，防止并发冲突
- OCR 结果本地缓存，同一文件不重复 OCR

### 7.3 部署

- 默认：本地单机或内网部署，冷启动一键/输入 URL 即可使用
- 可选：Supabase/Vercel 等外部服务（非内网场景，不作为默认要求）
...
- Token 预估：生成前预估 token/费用；生成后显示实际消耗
...
**支持 Provider（国内）：** Kimi、Minimax、GLM、小米 MiMo（默认候选）

**Fallback 机制：** 配额错误自动切换下一个；其他错误指数退避后重试；用户可调整顺序。

### 7.6 资源库优化

- 段落级 MD5 去重 + 磁盘缓存；资源库未更新时复用处理结果
...
> **T0** = 开发正式启动日（待定）。Gantt 图中的绝对日期仅供渲染使用，**不代表实际承诺时间**。

```mermaid
gantt
    title 专利审查助手 产品路线图（T0 = 开发启动日）
    dateFormat  YYYY-MM
    section v0.1.0（MVP · T0 ~ T0+2M）
    案件基线 + 文献清单 + 时间轴校验    :done,    2024-01, 1M
    文档解析 + OCR 扫描PDF支持          :active,  2024-01, 1M
    权利要求特征拆解（Claim Chart）      :active,  2024-01, 1M
    文献对照·新颖性框架                  :         2024-01, 1M
    Mock 演示模式（预置3案例）            :         2024-02, 1M
    工作台壳子（全模块占位）              :         2024-02, 1M
    审查意见素材草稿（壳子）              :         2024-02, 1M
    模型配置 + 安全设置                  :         2024-02, 1M
    section v0.2.0（迭代一 · T0+2M ~ T0+4M）
    创造性三步法·Step 3 技术启示分析     :         2024-03, 2M
    区别特征候选·完整闭环                :         2024-03, 2M
    多轮对话·各模块独立上下文            :         2024-04, 1M
    RAG 本地向量库                       :         2024-04, 1M
    section v0.3.0（迭代二 · T0+4M ~ T0+6M）
    形式缺陷辅助分析                     :         2024-05, 2M
    审查意见素材草稿·完整实现            :         2024-05, 2M
    答复审查意见·超范围辅助判断          :         2024-06, 1M
    多模态：图表/结构图识别              :         2024-06, 2M
    section v1.0.0（正式版 · T0+6M ~ T0+8M）
    全流程闭环测试 + Evaluation Set      :         2024-07, 1M
    内网一键部署方案                     :         2024-07, 1M
    审查员培训材料 + README              :         2024-08, 1M
```
...
- **API 网关：** 代理转发各 AI Provider 请求；在此层实施脱敏拦截
- **文档解析：** PDF → 文本（pdfjs-dist）；DOCX → 文本（mammoth）；OCR fallback（服务端 Tesseract）
- **向量检索（RAG）：** 本地轻量向量库（v0.2.0 实现）
- **加密：** API Key AES-256-GCM + PBKDF2 浏览器指纹派生密钥
...
3. 填写申请号、申请日、优先权日（如有）
4. 上传至少一篇对比文件，APP 自动提取公开日并完成时间轴校验
5. APP 生成 Claim Chart（权利要求1的特征拆解 + 说明书出处标注）
6. APP 对对比文件中每个技术特征进行新颖性对照，输出公开状态 + Citation
7. 审查员可编辑 Claim Chart 和对照表，并通过对话框追问
8. 导出对照表为 HTML 文件

以上8步形成 v0.1.0 的完整 MVP 闭环。其余模块在界面上有占位入口但可以是壳子。
