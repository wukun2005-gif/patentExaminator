...
| 项目 | 内容 |
|------|------|
| 产品名称 | 专利复审 AI 助手（Patent Reexamination AI Assistant） |
| 版本 | v0.1.0 |
| 产品形态 | Web App（v0.1.0 本地/内网部署 → 验证后迁移国内云部署） |
| 目标用户 | 发明专利实质审查员 |
| 核心价值 | 帮助审查员解析审查意见通知书、映射申请人答辩理由，并生成逐条回应格式的复审意见草稿 |
| 明确非目标 | 不替代已有审查系统、案件管理、发文系统；不作出法律结论；不再覆盖初审从零分析场景 |
...
v0.1.0 自 B-008 起定位为**复审 AI 助手**。标准输入为申请文件、审查意见通知书、意见陈述书，以及可选的修改后权利要求书。核心流程变为：
...
原初审式"从零生成审查意见素材"不属于当前版本范围；保留的 Claim Chart、新颖性、创造性和文献管理能力均作为复审回应链路的上游事实材料。
...
**背景：** 审查员手上有一件电子领域发明专利申请（扫描 PDF 格式），以及从检索系统检索到的两篇对比文件（一篇专利、一篇期刊论文）。想在开会前 30 分钟内整理出权利要求技术特征和初步文献对照结果。

**Step 1 — 打开工具**
审查员在浏览器里输入内网地址，工具秒开。顶部显示"演示模式"横幅，她点击切换为"真实模式"，弹出安全提示，她确认 API Key 已配置，点击继续。
...
>
> **Agent ↔ 功能模块映射：** 文档解析 Agent → §6.3；文档解读 Agent → §6.3.5；创新点研读 Agent → §6.4/6.5；创造性分析 Agent → §6.5.2；简述 Agent → §6.7；审查意见素材 Agent → §6.8；HTML 格式转换 Agent → §6.10。
>
> **Agent ↔ Design/Dev Plan ID 映射：** 本图 Agent 为逻辑角色名。Design/Dev Plan 中以技术 ID 引用：文档解读 Agent → `interpret`（`moduleScope: "case"`）；创新点研读 Agent → `claim-chart` + `novelty`（两个 Agent）；创造性分析 Agent → `inventive`；简述 Agent → `summary`；审查意见素材 Agent → `draft`；另有 `chat` Agent（各模块独立对话）和 Orchestrator（后端 `server/src/lib/orchestrator.ts`，非独立 Agent）。HTML 格式转换不作为 Agent，由导出模块直接处理。
...
v0.1.0 不接入任何官方案件系统，所有字段手动录入或从文件提取。
...
- 对比文件数量上限：10 篇（超出时阻止继续添加，提示用户合并或分批分析）
...
**RAG 支持（v0.2.0，已实现）：** 法规知识库 RAG 系统，全部运行在服务端。支持多格式文件上传（PDF/TXT/MD/DOCX/JSON/Excel/CSV/PNG）和在线 URL 抓取。文本经清洗、规范化、去重后切片，通过混合检索（语义 + BM25 RRF 融合）+ 三级重排序（远程 API bge-reranker-v2-m3 → 本地 cross-encoder bge-reranker-base → 本地启发式）+ 法条知识图谱扩展检索相关法规，注入 Agent prompt 减少幻觉。Embedding 为可选项：配置远程 API 时启用语义检索，未配置时降级为纯 BM25 关键词匹配。Query 扩展（跨语言 + 法律同义词 + 法条图谱）在服务端完成。用户在设置页面"知识库"tab 配置远程 Embedding 和 Re-ranker API。详见 B-018/B-018-1/B-018-2。
...
**v0.1.0 壳子要求：** 左侧导航显示"案件历史"入口，点击后展示已有案件列表（从后端 SQLite `cases` 表读取），可点击载入任一案件继续工作。数据层完整（所有 ChatMessage 和 PatentCase 已写入后端 SQLite），缺的是浏览 UI。

**功能要求：**

| 能力 | v0.1.0 状态 | v0.2.0 状态 |
|------|------------|------------|
| 交互记录持久化 | ✅ 实现 — 所有 ChatMessage 写入后端 SQLite，按 `caseId + moduleScope + createdAt` 存储 | — |
...
| 案件恢复（从后端 SQLite 载入已有案件继续工作） | ✅ 实现 — 后端 SQLite 持久化 + 案件载入 | — |
...
| 要求 | 说明 |
|------|------|
| 数据不外发原则 | 案件数据保存在应用内（后端 SQLite），外部 AI 调用前必须显示将发送的内容，用户确认后才发送 |
...
- SQLite 存储生命周期：OCR 缓存 7 天自动过期；用户可在设置中一键清除所有本地数据

### 7.3 部署（三阶段路线）

**阶段一：本地验证（v0.1.0，当前）**
- 默认：本地单机或内网部署，冷启动一键/输入 URL 即可使用
- 受限网络环境（§7.1 仅国内 URL）下必须使用纯本地部署，不使用任何海外外部服务

**阶段二：远程试用（v0.1.0 本地验证通过后，与 v0.2.0 并行）**
- 本地验证通过后，部署至 Vercel（前端）+ Supabase（后端 + 数据库）免费套餐，快速获得公网可访问的远程试用环境
- **目的：** 让更多审查员和决策者远程试用产品、收集反馈，无需安装任何软件；不涉及正式合规部署
- **前端：** Vercel 免费套餐托管 React SPA + Serverless Functions（替代自托管 Express 的 API 路由）
- **后端：** Supabase 免费套餐提供 PostgreSQL 数据库（替代后端 SQLite，实现多用户数据持久化）+ Auth（可选，替代手动 HTTP Basic Auth）+ Edge Functions（可选）
- **迁移策略：** 当前架构为 Node Express 静态托管 + API，需适配 Vercel Serverless Functions 的无状态模型；Supabase 提供 JS SDK，前端代码改动量可控
- **限制：** Vercel/Supabase 均为海外服务，访问速度可能受限；免费套餐有额度限制（适合小规模试用，不适合生产使用）；此阶段**不处理**敏感的真实申请文件，仅用于功能演示和流程验证
- **试用规模：** 5–10 人小范围内部试用，收集产品反馈

**阶段三：国内云合规部署（v0.1.0 后，与 v0.2.0 并行）**
- 远程试用反馈确认产品可用后，迁移至国内云服务器（阿里云/腾讯云/华为云轻量应用服务器），绑定国内域名，提供合规的生产级访问
- 迁移成本低：应用核心逻辑不变，仅替换部署目标和数据库后端
- 推荐路线：阶段一本地验证 → 阶段二远程试用收集反馈 → 阶段三国内云合规部署
...
- ~~反馈系统~~：已删除（用户要求），满意度评估改为 Metrics 体系自动指标（Groundedness、Faithfulness 等）
...
**支持 Provider（国内）：** Kimi、GLM、Minimax、小米 MiMo（默认候选）、Deepseek
...
| 用户满意度 | 通过 Metrics 体系自动指标衡量（Groundedness、Faithfulness、Answer Correctness 等） | 离线评估 + Dashboard |
...
| 接入检索系统 vs 用户上传 | 用户上传 | v0.1.0 不接入内部检索系统；设计边界 |
| 云平台选择 | 三阶段：本地 → Vercel+Supabase 试用 → 国内云合规 | 先本地验证；再用 Vercel+Supabase 免费套餐快速远程试用收集反馈；最后迁移国内云轻量服务器做合规部署 |
...
gantt
    ...
    创造性三步法（核心）                  :         2026-06, 1M
    Mock 演示模式（预置3案例）            :         2026-06, 1M
    工作台壳子（全模块占位）              :         2026-06, 1M
    审查意见素材草稿（壳子）              :         2026-06, 1M
    模型配置 + 安全设置                  :         2026-06, 1M
    section 阶段二（远程试用 · 与 v0.2.0 并行 · 不阻塞功能迭代）
    Vercel + Supabase 适配与部署          :         2026-07, 0.5M
    远程试用·收集反馈                     :         2026-07, 1M
    section 阶段三（国内云合规部署 · 与 v0.2.0 并行）
    购买国内轻量服务器 + ICP备案 + 部署上线  :         2026-08, 0.5M
    section v0.2.0（迭代一 · T0+2M ~ T0+4M）
    创造性三步法·深度分析增强            :         2026-07, 2M
...
| 版本 | 启动时间 | 核心交付 | 明确不做 |
|------|---------|---------|---------|
| **v0.1.0** | T0 | OCR + 权利要求拆解 + 新颖性对照 + 创造性三步法可真实使用；Mock 演示模式；壳子功能 | RAG、多轮对话 |
| **阶段二（远程试用）** | 本地验证通过后（与 v0.2.0 并行） | Vercel + Supabase 免费套餐部署远程试用环境，5–10 人小范围试用收集反馈；不处理真实申请文件 | 生产级部署；敏感数据处理 |
| **阶段三（国内云合规）** | 远程试用反馈确认后（与 v0.2.0 并行） | 国内云服务器部署上线（阿里云/腾讯云/华为云），绑定国内域名，含访问认证（HTTP Basic Auth/IP 白名单）+ HTTPS；每位审查员独立实例 | 多用户隔离（留到 v1.0.0） |
| **v0.2.0** | T0 + 2M | 创造性三步法深度增强；**RAG 法规知识库（✅ 已完成）**；~~反馈按钮~~（已删除）；案件历史完整实现；多轮对话 | 形式缺陷分析、答复审查意见 |
...
**阶段二（远程试用，Vercel + Supabase）：**

```
Vercel（前端 + Serverless Functions）
  ├── React SPA 静态托管
  └── Serverless Functions（/api/ai/run 等 API 路由）

Supabase（后端服务）
  ├── PostgreSQL 数据库（替代后端 SQLite，多用户数据持久化）
  ├── Auth（可选，用户认证）
  └── Edge Functions（可选）
```

免费套餐适合 5–10 人小范围试用；不处理敏感的真实申请文件。
...
应用核心逻辑不变，仅替换部署目标和数据库后端。认证需求需提前完成。

---

## 13. 明确非目标（Non-Goals）

1. 替代已有审查系统、案件管理、发文系统
...
| Deepseek API 文档（https://platform.deepseek.com）| 同上 |
| MiMo Token Plan 文档（https://token-plan-cn.xiaomimimo.com）| 小米 MiMo Token Plan，默认候选 Provider |
...
| 术语 | 定义 |
|------|------|
| OA | Office Action，审查意见通知书，发文系统生成的格式文件 |
...
9. APP 进行创造性三步法分析（确定最接近现有技术 → 区别特征 + 实际技术问题 → 技术启示判断），结论标注"候选/待审查员确认"
10. 导出对照表为 HTML 文件

以上 10 步形成 v0.1.0 的核心 MVP 闭环。

此外，v0.1.0 必须交付以下附加项（对应 §6.10/§6.11/§6.12）：

11. HTML 导出文件名自动生成（`申请号_发明名称_内容类型_日期.html`），可直接打印
12. 案件历史列表（壳子）：可浏览已有案件并载入继续工作
13. Mock 演示模式（默认开启，零 Token、零联网，完整覆盖上述 1–12），内置 G1/G2/G3 预置案例，作为独立验收项（见 §6.11）
