...
| **v0.2.0** | T0 + 2M | 创造性三步法深度增强；**RAG 法规知识库（✅ 已完成）**；反馈按钮 UI（✅ 已完成）；案件历史完整实现；多轮对话 | 形式缺陷分析、答复审查意见 |
