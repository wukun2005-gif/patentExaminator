...
>
> **Agent ↔ Design/Dev Plan ID 映射：** 本图 Agent 为逻辑角色名。Design/Dev Plan 中以技术 ID 引用：文档解读 Agent → `interpret`（`moduleScope: "case"`）；创新点研读 Agent → `claim-chart` + `novelty`（两个 Agent）；创造性分析 Agent → `inventive`；简述 Agent → `summary`；审查意见素材 Agent → `draft`；另有 `chat` Agent（各模块独立对话）和 Orchestrator（前端 `AgentClient` + 后端 `AI Gateway`，非独立 Agent）。HTML 格式转换不作为 Agent，由导出模块直接处理。
...
**RAG 支持（v0.2.0，已实现）：** 法规知识库 RAG 系统，支持多格式文件上传（PDF/TXT/MD/DOCX/JSON/Excel/CSV/PNG）和在线 URL 抓取。文本经清洗、规范化、去重后切片，通过混合检索（语义 + BM25 RRF 融合）+ 多信号重排序 + 法条知识图谱扩展检索相关法规，注入 Agent prompt 减少幻觉。用户在设置页面"知识库"tab 配置，支持本地 BGE 模型或远程 embedding API。详见 B-018/B-018-1/B-018-2。
...
- **后端：** Supabase 免费套餐提供 PostgreSQL 数据库（替代浏览器端 IndexedDB，实现多用户数据持久化）+ Auth（可选，替代手动 HTTP Basic Auth）+ Edge Functions（可选）
...
- **向量检索（RAG）：** 法规知识库 RAG 系统（v0.2.0 已实现），支持混合检索 + 重排序 + 法条知识图谱扩展，详见 `client/src/lib/knowledge/`
