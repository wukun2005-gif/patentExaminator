...
const QUICK_STEPS: Step[] = [
  ...
  },
  {
    title: "浏览各功能页面",
    content:
      "加载完成后，左侧导航栏可以切换：案件基本信息 → 文档导入 → 文献清单 → 文档解读 → 权利要求特征表 → 新颖性对照 → 创造性分析 → 形式缺陷 → 导出。每个页面都有对应的预置数据。"
  },
  {
    title: "切换模式",
    content:
...
const GUIDE_STEPS: Step[] = [
  ...
  {
    title: "第7步：生成权利要求特征表（特征分解）",
    content:
      "进入左侧导航「权利要求特征表」页面，选择要分析的权利要求（默认为权利要求1），点击「生成特征分解」按钮。系统会将权利要求拆解为技术特征 A、B、C…，每个特征关联说明书段落。",
  ...
  },
  {
    title: "第11步：导出审查报告",
    content:
      "进入左侧导航「导出」页面，选择 HTML 或 Markdown 格式，点击导出。报告包含案件信息、权利要求特征表、新颖性对照、创造性分析、缺陷清单等完整内容。",
